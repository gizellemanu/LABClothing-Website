...
export class ColectioneditComponent implements OnInit{
  ...
  public save() {this.http.put('http://localhost:3000/colecoes/' + this.colecao.id, this.colecao).subscribe(data => {});}
  
  public onSubmit() { this.save(); }
  ...
  collectionlist () { this.router.navigate(['/sidebar/navbar/colectionlist']); }
}
